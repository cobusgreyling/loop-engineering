...
export function heuristicRoute(input) {
    ...
    let tier = 'balanced';
    if (/\b(typo|format|whitespace|rename|list files|noop|one-line)\b/.test(text) &&
        files <= 2) {
        tier = 'nano';
    }
    else if (input.level === 'L1' ||
        /\b(triage|changelog|label|classify|report-only|draft notes)\b/.test(text)) {
        tier = 'fast';
    }
    if (/\b(refactor|architecture|root cause|multi-file|ambiguous|redesign)\b/.test(text) ||
        files >= 8) {
        tier = 'frontier';
    }
    if (/\b(verif|security|maker.?checker|policy|audit|review the patch)\b/.test(text) ||
        input.level === 'L3') {
        tier = 'reasoning';
    }
    const needsMakerChecker = tier === 'reasoning' || tier === 'frontier' || input.level === 'L3' ? 0.85 : 0.15;
...
export async function routeTask(input, opts = {}) {
