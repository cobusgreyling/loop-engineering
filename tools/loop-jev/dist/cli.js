...
import { guardText, guardExitCode } from './guard.js';
...
function parseFlags(argv) {
    const flags = { help: false, json: false, noFallback: false };
    for (let i = 0; i < argv.length; i += 1) {
        ...
        if (a === '--help' || a === '-h')
            ...
            flags.message = next();
        else if (!a.startsWith('-') && !flags.goal && !flags.text)
            flags.text = a;
        else
...
async function cmdRoute(flags) {
    const goal = flags.goal || flags.text;
    if (!goal)
        throw new Error('route requires --goal');
    const files = flags.files ? flags.files.split(',').map((s) => s.trim()).filter(Boolean) : undefined;
    const decision = await routeTask({ goal, pattern: flags.pattern, level: flags.level, files, notes: flags.notes }, { allowFallback: !flags.noFallback });
    if (flags.json)
        printJson(decision);
    else {
        const hint = TIERS[decision.tier];
        console.log(`${decision.uncertain ? 'UNCERTAIN' : 'ROUTE'} ${decision.tier} → ${decision.model}  conf=${decision.confidence.toFixed(2)}  source=${decision.source}`);
        console.log(`${hint.use} (${hint.cost})`);
        if (decision.needsMakerChecker >= 0.7)
            console.log('Recommend maker/checker before apply.');
    }
    return 0;
}
async function cmdRetrieve(flags) {
    if (!flags.query)
        throw new Error('retrieve requires --query');
    if (!flags.passages)
        throw new Error('retrieve requires --passages <file.json>');
    const passages = await loadPassages(flags.passages);
    const decision = await retrieveContext({ query: flags.query, passages, top: flags.top }, { allowFallback: !flags.noFallback });
    if (flags.json)
        printJson(decision);
    else {
        console.log(`RETRIEVE kept ${decision.kept.length}/${decision.ranked.length}  contains=${decision.containsAnswer.toFixed(2)}  source=${decision.source}`);
...
async function cmdGuard(flags) {
    ...
    const side = (flags.side || 'input');
    if (side !== 'input' && side !== 'output')
        throw new Error('--side must be input or output');
    const policy = (flags.policy || 'strict');
    if (policy !== 'strict' && policy !== 'permissive')
        throw new Error('--policy must be strict or permissive');
    const decision = await guardText({ text, side, policy }, { allowFallback: !flags.noFallback });
    if (flags.json)
        printJson(decision);
    else {
        console.log(`${decision.action.toUpperCase()} [${decision.topHazard}=${decision.topProbability.toFixed(2)} sev=${decision.severity.toFixed(2)}] source=${decision.source}`);
    }
    return guardExitCode(decision.action);
}
async function cmdClassify(flags) {
    if (!flags.trace)
        throw new Error('classify requires --trace <file>');
    const input = await loadClassifyInput(flags.trace);
    const decision = await classifyTrace(input, { allowFallback: !flags.noFallback });
    if (flags.json)
        printJson(decision);
    else {
        console.log(`${decision.failureMode.toUpperCase()} conf=${decision.confidence.toFixed(2)} escalate=${decision.shouldEscalate.toFixed(2)} source=${decision.source}`);
    }
...
async function cmdTurn(flags) {
    const goal = flags.goal || flags.text;
    if (!goal)
        throw new Error('turn requires --goal');
    const files = flags.files ? flags.files.split(',').map((s) => s.trim()).filter(Boolean) : undefined;
    const decision = await assessTurn({
        ...
        files,
        notes: flags.notes,
        policy: flags.policy || 'strict',
    }, { allowFallback: !flags.noFallback });
    if (flags.json)
        printJson(decision);
    else {
        console.log(`TURN ${decision.action.toUpperCase()}  ${decision.route.tier} → ${decision.route.model}  source=${decision.source}`);
        console.log(`guard ${decision.guard.topHazard}=${decision.guard.topProbability.toFixed(2)}  route conf=${decision.route.confidence.toFixed(2)}`);
    }
    return guardExitCode(decision.action);
}
async function main() {
