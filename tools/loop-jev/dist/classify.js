...
function clampMode(value) {
    if (value && value in FAILURE_CRITERIA)
        return value;
    return 'other';
}
function compactEvents(events, limit = 40) {
...
export function heuristicClassify(input) {
    ...
    const uniqueTools = new Set(events.filter((e) => (e.type ?? '').includes('tool')).map((e) => e.detail ?? e.primitive ?? ''));
    let mode = 'healthy';
    if (types.some((t) => t.includes('budget')))
        mode = 'budget_burn';
    else if (types.some((t) => t.includes('policy')))
        mode = 'policy_denied';
    else if (types.some((t) => t.includes('verification.fail') || t.includes('verifier')))
        mode = 'verifier_fail';
    else if (toolCalls >= 6 && uniqueTools.size <= 2)
        mode = 'tool_loop';
    else if (/\b(stagnat|same error|no progress)\b/.test(blob))
        mode = 'stagnation';
    else if (/\b(drift|abandoned the goal)\b/.test(blob))
        mode = 'instruction_drift';
    else if (/\bhallucin/.test(blob))
        mode = 'hallucination';
    else if (/\bworktree\b/.test(blob) && /\b(collision|conflict|lock)\b/.test(blob))
        mode = 'worktree_collision';
    else if (types.some((t) => t === 'error' || t.endsWith('.error')))
        mode = 'other';
...
export async function classifyTrace(input, opts = {}) {
...
export function classifyExitCode(decision) {
    if (decision.failureMode === 'healthy' && decision.shouldEscalate < 0.5)
        return 0;
    return 2;
}
