...
export function parseRepairContract(value, source = 'repair contract') {
    ...
    if (maxAttempts === 0)
        throw new Error(`${source}.maxAttempts must be greater than zero.`);
    return {
        version: 1,
        labels: {
...
function parseTarget(value, source) {
    ...
    if (parsed.number === 0)
        throw new Error(`${source}.number must be greater than zero.`);
    if (target.changedPaths !== undefined)
        parsed.changedPaths = strings(target.changedPaths, `${source}.changedPaths`);
    if (target.linkedPullRequest !== undefined) {
        parsed.linkedPullRequest = integer(target.linkedPullRequest, `${source}.linkedPullRequest`);
    }
    parsed.reproduction = optionalEnum(target.reproduction, ['confirmed', 'unknown', 'failed'], `${source}.reproduction`);
    if (target.headSha !== undefined)
        parsed.headSha = string(target.headSha, `${source}.headSha`);
    if (target.branchOwned !== undefined) {
        if (typeof target.branchOwned !== 'boolean')
            throw new Error(`${source}.branchOwned must be a boolean.`);
        parsed.branchOwned = target.branchOwned;
    }
    if (target.reviewActionable !== undefined) {
        if (typeof target.reviewActionable !== 'boolean')
            throw new Error(`${source}.reviewActionable must be a boolean.`);
    ...
    if (target.check !== undefined) {
        ...
        const failureClass = optionalEnum(check.failureClass, ['deterministic', 'unknown', 'flake', 'infrastructure'], `${source}.check.failureClass`);
        if (!status || !failureClass)
            throw new Error(`${source}.check requires status and failureClass.`);
...
export function parseRepairEvidence(value, source = 'repair evidence') {
    const root = record(value, source);
    if (root.version !== 1)
        throw new Error(`${source}.version must be 1.`);
    ...
    if (!Array.isArray(root.targets))
        throw new Error(`${source}.targets must be an array.`);
    ...
    if (new Set(identities).size !== identities.length)
        throw new Error(`${source}.targets contains duplicates.`);
    return {
        version: 1,
        observedAt: timestamp(root.observedAt, `${source}.observedAt`),
        repository: string(root.repository, `${source}.repository`),
        pauseIssues: root.pauseIssues,
        targets,
    };
}
function attempts(target, contract) {
...
function priorityScore(target, contract) {
...
function candidateFor(target, attempt, contract) {
    const mode = target.type === 'pull-request' && !target.branchOwned ? 'propose' : 'change';
...
export function evaluateRepair(contract, evidence) {
    ...
    for (const target of evidence.targets) {
        ...
        if (!candidateFor(target, 0, contract))
            continue;
...
export async function loadRepairContract(path) {
    return parseRepairContract(parse(await readFile(path, 'utf8')), path);
}
export async function loadRepairEvidence(path) {
