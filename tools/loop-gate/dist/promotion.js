...
function optionalMaxAge(value, path) {
    if (value === undefined)
        return undefined;
    const number = requireNumber(value, path);
    if (number === 0)
...
export function parsePromotionContract(value, source = 'promotion contract') {
    ...
    return {
        ...
        pullRequest: {
            ...
            requireResolvedThreads: requireBoolean(pullRequest.requireResolvedThreads, `${source}.pullRequest.requireResolvedThreads`),
        ...
        e2e: {
            ...
            pathSuites: e2e.pathSuites === undefined
                ? undefined
                : (() => {
                    if (!Array.isArray(e2e.pathSuites))
                        throw new Error(`${source}.e2e.pathSuites must be an array.`);
        ...
        manualAcceptance: {
            mode: mode,
            riskLevels: manualAcceptance.riskLevels === undefined
                ? undefined
...
function parseEvidenceItems(root, source) {
    if (root.version !== 1)
        throw new Error(`${source}.version must be 1.`);
    const pullRequest = requireRecord(root.pullRequest, `${source}.pullRequest`);
    ...
    const parsed = {
        ...
        pullRequest: {
            ...
            approvalSha: pullRequest.approvalSha === undefined
                ? undefined
        ...
        e2e: e2e.map((item, index) => {
            ...
            return {
                ...
                datasetVersion: suite.datasetVersion === undefined
                    ? undefined
    ...
    if (root.testData !== undefined) {
        ...
        parsed.testData = {
            kind: kind,
            version: testData.version === undefined ? undefined : requireString(testData.version, `${source}.testData.version`),
    ...
    if (root.manualAcceptance !== undefined) {
        ...
        parsed.manualAcceptance = {
            ...
            actor: acceptance.actor === undefined
                ? undefined
...
export function parsePromotionEvidence(value, source = 'promotion evidence') {
    return parseEvidenceItems(requireRecord(value, source), source);
}
export async function loadPromotionContract(file) {
    let raw;
    try {
...
export async function loadPromotionEvidence(file) {
    let raw;
    try {
        raw = await readFile(file, 'utf8');
    }
...
function stale(completedAt, observedAt, maxAgeHours) {
    if (maxAgeHours === undefined)
        return false;
    return Date.parse(observedAt) - Date.parse(completedAt) > maxAgeHours * 60 * 60 * 1000;
}
function issue(code, stage, kind, message) {
    return { code, stage, kind, message };
}
function acceptanceRequired(contract, riskLevel) {
    if (contract.manualAcceptance.mode === 'always')
        return true;
    if (contract.manualAcceptance.mode === 'never')
        return false;
    return (contract.manualAcceptance.riskLevels ?? []).includes(riskLevel);
}
export function evaluatePromotion(contract, evidence, pathPolicy) {
    const problems = [];
    const pr = evidence.pullRequest;
    const sha = pr.headSha;
    if (pr.draft)
        problems.push(issue('draft', 'intake', 'waiting', 'Pull request is still a draft.'));
    ...
    if (pr.attempts >= contract.pullRequest.maxAttempts) {
        problems.push(issue('attempt-limit', 'intake', 'human-required', `Attempt limit reached (${pr.attempts}/${contract.pullRequest.maxAttempts}).`));
    }
    const missingLabels = contract.authorization.labels.filter((label) => !pr.labels.includes(label));
    if (missingLabels.length > 0) {
        problems.push(issue('authorization-label', 'intake', 'human-required', `Missing authorization label(s): ${missingLabels.join(', ')}.`));
    }
    if (pathPolicy) {
        const decision = checkGate({ config: pathPolicy, action: 'auto-merge', paths: pr.changedPaths });
        if (!decision.allowed)
            problems.push(issue(`path-${decision.trigger}`, 'policy', 'human-required', decision.reason));
    }
    ...
    if (pr.approvals < contract.authorization.minApprovals) {
        problems.push(issue('approvals', 'review', 'human-required', `Approvals ${pr.approvals}/${contract.authorization.minApprovals}.`));
    }
    else if (contract.authorization.minApprovals > 0 && pr.approvalSha !== sha) {
        problems.push(issue('approval-sha', 'review', 'human-required', 'Counted approvals are not for the current PR HEAD SHA.'));
    }
    ...
    if (contract.pullRequest.requireResolvedThreads && pr.unresolvedThreads > 0) {
        problems.push(issue('review-threads', 'review', 'human-required', `${pr.unresolvedThreads} review thread(s) remain unresolved.`));
    }
    ...
    if (contract.deployment.required) {
        ...
        if (!deployment) {
            ...
            if (deployment.environment !== contract.deployment.environment) {
                problems.push(issue('deployment-environment', 'deployment', 'blocked', `Expected environment ${contract.deployment.environment}, got ${deployment.environment}.`));
            }
            ...
            if (deployment.status !== 'success') {
                problems.push(issue(`deployment-${deployment.status}`, 'deployment', deployment.status === 'pending' ? 'waiting' : 'blocked', `Deployment status is ${deployment.status}.`));
            }
    ...
    if (contract.testData.required) {
        ...
        if (!data) {
            ...
            if (!contract.testData.allowedKinds.includes(data.kind)) {
                problems.push(issue('test-data-kind', 'test-data', 'human-required', `Dataset kind ${data.kind} is not permitted.`));
            }
            ...
            if (data.status !== 'success') {
                problems.push(issue(`test-data-${data.status}`, 'test-data', data.status === 'pending' ? 'waiting' : 'blocked', `Test-data reset/seed status is ${data.status}.`));
            }
        }
    }
    ...
    for (const requiredSuite of requiredSuites) {
        ...
        if (suite.status !== 'success') {
            problems.push(issue(`e2e-${suite.status}`, 'e2e', suite.status === 'pending' ? 'waiting' : 'blocked', `E2E suite ${requiredSuite} is ${suite.status}.`));
        }
        if (contract.e2e.requireDatasetMatch &&
            evidence.testData?.version &&
            suite.datasetVersion !== evidence.testData.version) {
            problems.push(issue('e2e-dataset', 'e2e', 'blocked', `E2E suite ${requiredSuite} used a different dataset version.`));
        }
    ...
    if (acceptanceRequired(contract, pr.riskLevel)) {
        ...
        if (!acceptance) {
            ...
            if (acceptance.status !== 'success') {
                problems.push(issue(`acceptance-${acceptance.status}`, 'acceptance', acceptance.status === 'pending' ? 'human-required' : 'blocked', `Manual acceptance status is ${acceptance.status}.`));
            }
            if (!acceptance.actor) {
