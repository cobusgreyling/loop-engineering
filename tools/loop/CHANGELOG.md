# Changelog

## 0.2.1

- Pass-through `loop jev …` → `@cobusgreyling/loop-jev` (TypeSafe System One: route, retrieve, guard, classify)

## 0.2.0

- Default `--tool` is `claude` (was `grok`)
- Week-one copy and wizard default to Claude Code
- Wizard pain #8: thin-loop (GitHub Action, no extra files)
- Companion upsell (`--with-foundry` / memory / fleet) is opt-in, not printed on every init

## 0.1.2

- Restore public name `@cobusgreyling/loop` after temporary loop-cli experiment
- Docs and bin `loop` aligned with live 0.1.0

## 0.1.1

- Confirm public package name `@cobusgreyling/loop` (bin `loop`)
- Set `main` to `dist/cli.js`
- Docs/showcase use `@cobusgreyling/loop` as the front door

## 0.1.0

- Initial unified CLI front door: pass-through + doctor/status/badge/wizard
